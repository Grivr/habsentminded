...
import           Data.ConfigFile       (emptyCP, get, optionxform, readfile)
import           Data.List             (elemIndex, insertBy, sort)
import           Data.List.Split       (chunksOf, dropFinalBlank, keepDelimsR,
                                        split, whenElt)
...
decodeText config input=
  let
    ...
    pointers =  getPointers pointerTable
    hTree = buildTree treeLeft treeRight charMap
    compressedBlocks = map (getBitStream input (textBase config)) pointers
    decodedMsgs = concatMap (head . splitFourScreens (endOfScreen config) . decode hTree) (init compressedBlocks)--each block contains 4 screen messages
    decodedLvls = takeWhile (/= endOfScreen config) $ decode hTree (last compressedBlocks) --last block has level names terminated with EOS

    in decodedMsgs ++ decodedLvls ++  [endOfScreen config]
  ...
  if action == Decode
    then do
      ...
      let inputU8 =  Bs.unpack input
          ...
          hdr = Bs.pack $ treeLeft ++ treeRight ++ charMap ++ pointers --assemble back
          txt = Bi.toByteString (Bi.pack encodedStr :: Bi.Bitstream Bi.Right)
      Bs.writeFile outputFileName $ Bs.append hdr txt
